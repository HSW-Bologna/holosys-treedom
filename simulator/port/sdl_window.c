...
lv_display_t *sdl_window_create(int32_t hor_res, int32_t ver_res) {
    ...
    /*LV_DISPLAY_RENDER_MODE_DIRECT or FULL */
        ...
        uint32_t stride =
            lv_draw_buf_width_to_stride(lv_display_get_horizontal_resolution(disp), lv_display_get_color_format(disp));
...
static void flush_cb(lv_display_t *disp, const lv_area_t *area, uint8_t *px_map) {
    ...
    if (LV_SDL_RENDER_MODE == LV_DISPLAY_RENDER_MODE_PARTIAL) {
        ...
        uint32_t px_map_stride =
            lv_draw_buf_width_to_stride(lv_area_get_width(area), lv_display_get_color_format(disp));
...
#error("Unsupported color format")
    ...
    dsc->texture = SDL_CreateTexture(dsc->renderer, px_format, SDL_TEXTUREACCESS_STATIC, hor_res, ver_res);
